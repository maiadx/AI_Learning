...
max_iters = 500 # training iters
eval_interval = 250
learning_rate = 3e-4
device = 'cuda' if torch.cuda.is_available() else 'cpu'
eval_iters = 200
n_embedding_dim = 384 # dimension table for 32 dimensional embeddings
n_head = 6
n_layer = 6
dropout = 0.2
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        super().__init__()
        self.heads = nn.ModuleList([Self_Attn_Head(head_size) for _ in range(num_heads)])
        self.proj = nn.Linear(head_size * num_heads, n_embedding_dim)
        self.dropout = nn.Dropout(dropout)
...
class Block(nn.Module):
    ...
    def __init__(self, n_embedding_dim, n_head):
        # n_embedding_dim: embedding dimension, n_head: the number of attention heads we'd like
        super().__init__()
        head_size = n_embedding_dim // n_head
...
class GPTLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embedding_dim, n_head=n_head) for _ in range(n_layer)])
        self.ln_f = nn.LayerNorm(n_embedding_dim) # final layer norm
        self.lm_head = nn.Linear(n_embedding_dim, vocab_size)

        # better init, not covered in tutorial vid
        self.apply(self._init_weights)
        # how we initialize our weights changes how well the model is trained in its affinities.
    ...
    def forward(self, idx, targets=None):
        ...
        # idx and targets are booth (B, T) tensor of integers
...
open('llm-output.txt', 'w').write(decode(m.generate(context, max_new_tokens=10000)[0].tolist()))